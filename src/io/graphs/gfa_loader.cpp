    ...
    while (std::getline(in, line)) {
        ...
        if (line.rfind("P\t", 0) == 0) {
            ...
            for (const auto& token : segments) {
                ...
                if (orient != '+' && orient != '-') {
                    LOG_ERROR("Missing orientation for segment '" + token + "' in path '" +
                              path.name + "' at line " + std::to_string(line_no));
                    ok = false;
                    continue;
                }
            ...
            if (fields[3] != "*") {
                ...
                if (!path.steps.empty() && path.overlaps.size() + 1 != path.steps.size()) {
                    LOG_WARN("Path '" + path.name +
                             "' overlaps count mismatch; keeping raw overlaps");
                }
            }
