...
AlignmentQuantizerPtr make_alignment_quantizer(const AlignmentQuantizerConfig& config) {
    if (config.backend == "int16" || config.backend == "int8" || config.backend == "int" ||
        config.backend == "fixed" || config.backend.empty()) {
        AlignmentQuantizerConfig cfg = config;
        if (config.backend == "int8") cfg.target_bits = 8;
        if (cfg.target_bits != 8) cfg.target_bits = 16;
        return std::make_unique<IntAlignmentQuantizer>(cfg);
    }
    if (config.backend != "passthrough" && config.backend != "float32") {
