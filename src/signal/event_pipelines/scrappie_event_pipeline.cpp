...
RawView trim_raw_by_mad(const std::vector<float>& raw, int chunk_size, float quantile) {
  ...
  const int thresh_idx = std::clamp(static_cast<int>(quantile * nchunk), 0, std::max(nchunk - 1, 0));
...
std::vector<int> short_long_peak_detector(const std::vector<float>& t1, const std::vector<float>& t2,
  ...
  float peak_val[2] = {std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity()};
  int peak_idx[2] = {-1, -1};
  int masked_to[2] = {0, 0};
...
NormalizedSignal ScrappieEventPipeline::process(const io::RawRead& read) const {
  ...
  auto peaks = short_long_peak_detector(t1, t2, config_.window_length1, config_.window_length2,
  ...
  if (config_.norm_method == "median_mad") {
    result.samples =
        median_mad_normalize(event_means, config_.clip_outliers, config_.clip_min, config_.clip_max);
  } else {
    // Default to zscore
    result.samples =
        zscore_normalize(event_means, config_.clip_outliers, config_.clip_min, config_.clip_max);
  }
