...
std::vector<int> short_long_peak_detector(const std::vector<float>& t1, const std::vector<float>& t2,
                                          int s_len, int window_length1, int window_length2,
                                          float threshold1, float threshold2, float peak_height) {
...
RawHashEventPipeline::RawHashEventPipeline(EventPipelineConfig config) : config_(std::move(config)) {}
