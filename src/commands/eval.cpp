...
int handle_eval(const std::vector<std::string>& args) {
  ...
  const std::string format = parsed.values.count("format") ? parsed.values.at("format") : "summary";
  ...
  for (const auto& gt : truths) {
    ...
    if (it != mappings.end() && !it->second.empty()) {
      ...
      for (const auto& candidate : it->second) {
        ...
        double overlap_frac =
            computeOverlapFraction(gt.start, gt.end, candidate.target_start, candidate.target_end);
  ...
  if (format == "summary") {
    ...
    *out << "Correct position: " << correct_position << " (" << std::fixed << std::setprecision(1)
         << (100.0 * correct_position / total) << "%, within " << tolerance << "bp)\n";
    *out << "Correct overlap:  " << correct_overlap << " (" << std::fixed << std::setprecision(1)
         << (100.0 * correct_overlap / total) << "%, >=" << std::setprecision(0)
         << (min_overlap * 100) << "% overlap)\n";
    ...
    if (parsed.values.count("verbose")) {
      *out << "\n=== Per-Read Results ===\n";
      for (const auto& r : results) {
        ...
        *out << (pass ? "PASS" : "FAIL") << "  " << r.read_id << " (len=" << read_len << "bp)";
        if (!r.mapped) {
          *out << "  UNMAPPED";
        } else {
          *out << "  mapped=" << r.mapping.target_name << ":" << r.mapping.target_start << "-"
               << r.mapping.target_end << "(" << r.mapping.strand << ")"
               << "  truth=" << r.truth.reference << ":" << r.truth.start << "-" << r.truth.end
               << "(" << r.truth.strand << ")"
               << "  overlap=" << std::fixed << std::setprecision(1) << (r.best_overlap_frac * 100)
               << "%"
        ...
        if (!r.mapped || r.best_overlap_frac < min_overlap) {
          ...
          *out << r.read_id << " (len=" << read_len << "bp): ";
          if (!r.mapped) {
            ...
            *out << "overlap=" << std::fixed << std::setprecision(1) << (r.best_overlap_frac * 100)
                 << "%";
    ...
    *out << "read_id\tread_length\tmapped\tcorrect_target\tcorrect_strand\tcorrect_"
    ...
    for (const auto& r : results) {
      ...
      *out << r.read_id << "\t" << read_len << "\t" << (r.mapped ? "1" : "0") << "\t"
           ...
           << r.truth.end << "\t" << (r.mapped ? std::to_string(r.mapping.target_start) : "NA")
    ...
    *out << "    \"mapping_rate\": " << std::fixed << std::setprecision(4) << (1.0 * mapped / total)
         << ",\n";
