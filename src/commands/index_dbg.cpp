...
int handle_index_dbg(const std::vector<std::string>& args) {
    piru::cli::Parsed parsed;
    piru::cli::ParseConfig config;
    config.usage = "Usage: piru index-dbg [options]";
    config.options = {
        {'h', "help", false, "Show help"},
        {'r', "reads", true, "Input reads (fastq/bam)"},
        {'k', "kmer", true, "K-mer length"},
        {'t', "threads", true, "Worker threads"},
        {'p', "profile", false, "Emit timing profile (tree)"},
    };
    config.on_error = [](const std::string&) { std::cerr << "index-dbg: invalid option\n"; };
    ...
    PIRU_PROFILE_START(profile, "index-dbg");

    LOG_ERROR("index-dbg under construction.");
    PIRU_PROFILE_STOP(profile, "index-dbg");
