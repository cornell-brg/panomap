...
std::pair<std::uint32_t, std::uint32_t> signalPosToNodeOffset(
    ...
    return {it->node_id,
            static_cast<std::uint32_t>(signal_pos - it->base_start)};
}
...
BucketIndexResult bucketIndex(const FlatGraph& graph, const io::KmerModel& model,
    ...
    auto process_path = [&](std::size_t path_idx, LinearCoordVec& coords_out,
        ...
        for (std::size_t s = 0; s < num_steps; ++s) {
            ...
            coords_out[nid].emplace_back(
                path_idx, static_cast<std::int64_t>(path_len));
    ...
    if (config.executor) {
        ...
        config.executor->parallel_for(std::size_t{0},
            static_cast<std::size_t>(graph.pathCount()), std::size_t{1},
            [&](std::size_t path_idx) {
                process_path(path_idx, thread_coords.local(), thread_buckets.local());
            });
...
#ifdef PIRU_USE_TBB
    if (config.executor) {
        config.executor->parallel_for(std::size_t{0}, num_buckets, std::size_t{1},
                                      finalize_one);
    } else
#endif
    {
    ...
    result.seed_store = std::make_unique<BucketSeedStore>(
        std::move(finalized), bucket_bits,
        extractor.name(), std::move(params),
        max_freq, max_freq + 1, 1.0);

    result.seeds_total = total_entries;

    timing::stop("bucket:finalize");

    LOG_INFO("BucketIndex: " + std::to_string(atomic_seeds_total.load()) +
             " raw seeds, " + std::to_string(total_unique) + " unique hashes, " +
             std::to_string(total_entries) + " entries after dedup" +
             " (max_freq=" + std::to_string(max_freq) +
