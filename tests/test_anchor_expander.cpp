...
TEST_CASE("AnchorExpander: Empty input returns empty output") {
    // Node 0 has one coordinate on path 0 at position 100
    std::vector<std::vector<LinearCoordinate>> coords(1);
    coords[0] = {{0, 100}};

    AnchorExpander expander(coords);
    auto anchors = expander.expand({});

    CHECK(anchors.empty());
}

TEST_CASE("AnchorExpander: Single seed on single path produces one anchor") {
    // Node 0 has one coordinate on path 0 at position 100
    std::vector<std::vector<LinearCoordinate>> coords(1);
    coords[0] = {{0, 100}};
...
TEST_CASE("AnchorExpander: Single seed on multiple paths produces multiple anchors") {
...
TEST_CASE("AnchorExpander: Seed with no coordinates produces no anchors") {
...
TEST_CASE("AnchorExpander: Offset handling adds to ref_coord") {
...
TEST_CASE("AnchorExpander: Zero offset produces ref_coord equal to node start") {
...
TEST_CASE("AnchorExpander: Multiple seeds produce multiple anchors") {
...
TEST_CASE("AnchorExpander: Node appearing on same path multiple times produces multiple anchors") {
...
TEST_CASE("AnchorExpander: Invalid node_id is skipped") {
...
TEST_CASE("AnchorExpander: Mix of valid and invalid nodes") {
    std::vector<std::vector<LinearCoordinate>> coords(2);
    coords[0] = {{0, 100}};
...
TEST_CASE("AnchorExpander: Seed length propagates to anchor") {
    std::vector<std::vector<LinearCoordinate>> coords(1);
    coords[0] = {{0, 100}};
