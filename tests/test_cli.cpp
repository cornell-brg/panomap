...
TEST_CASE("CLI smoke test: piru index") {
    ...
    const std::string index_dir = output_base + ".piru";
    ...
    // Check that the three index files were created in the .piru directory
    CHECK(std::filesystem::exists(index_dir + "/piru_cli_test_index.piru.graph"));
    CHECK(std::filesystem::exists(index_dir + "/piru_cli_test_index.piru.signals"));
    CHECK(std::filesystem::exists(index_dir + "/piru_cli_test_index.piru.seeds"));
