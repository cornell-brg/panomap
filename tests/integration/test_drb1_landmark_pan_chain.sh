#!/bin/bash
# Integration test: DRB1 pangenome with landmark tokenizer + pan-chain backend
# Simulates 1000 reads, indexes with landmark k=4, maps with pan-chain, checks accuracy >= 90%
...
"$PANOMAP" index "$GFA" --tokenizer landmark --seed-k 4 -o "$TMPDIR/drb1.pirx" 2>/dev/null
"$PANOMAP" inspect "$TMPDIR/drb1.pirx" --dump-path-coords "$TMPDIR/nodes.tsv" 2>/dev/null
"$PANOMAP" map --index "$TMPDIR/drb1.pirx" "$TMPDIR/reads.blow5" --chainer pan-chain -o "$TMPDIR/out.gaf" 2>/dev/null
...
echo ""
echo "Result: recall=$RECALL% mapping_accuracy=$MAP_ACC% ($CORRECT/$TOTAL correct, $WRONG wrong) [landmark k=4 pan-chain]"
