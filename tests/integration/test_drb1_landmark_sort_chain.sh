#!/bin/bash
# Integration test: DRB1 pangenome with landmark tokenizer + sort-chain backend
# Simulates 1000 reads, indexes with landmark k=4, maps with sort-chain, checks accuracy >= 90%
...
echo ""
echo "Result: recall=$RECALL% mapping_accuracy=$MAP_ACC% ($CORRECT/$TOTAL correct, $WRONG wrong) [landmark k=4 sort-chain]"
