#!/bin/bash
# Integration test: COVID linear reference mapping accuracy
# Maps 1000 real r9.4 reads, compares against pre-generated minimap2 truth PAF.
# Tests linear reference path (single-node GFA, interval dedup).
...
echo ""
echo "Result: recall=$RECALL% mapping_accuracy=$MAP_ACC% ($CORRECT/$TOTAL correct, $WRONG wrong) [covid r9.4 linear]"
...
if [ "$MAP_ACC" -lt "$MIN_PRECISION" ]; then
  echo "FAIL: mapping_accuracy $MAP_ACC% < ${MIN_PRECISION}%"
  FAIL=1
fi
