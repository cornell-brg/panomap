...
public:
    explicit ResultFormatter(const index::AlnGraph& graph, ResultFormatterConfig config = {});

    /// Format mappings to AlignmentResult vector.
    /// Returns one result per mapping (up to max_secondary + 1).
    std::vector<io::AlignmentResult> format(const ReadMapResult& map_result,
                                            const std::string& read_id,
                                            std::size_t read_length) const;

private:
    /// Format a single Mapping to AlignmentResult.
    io::AlignmentResult formatMapping(const Mapping& mapping, const std::string& read_id,
